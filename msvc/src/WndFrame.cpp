...
//
// Win32 constants for this application
//
...
LRESULT CALLBACK WndFrame::frameWndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_TOOLS_EDIT_LEVEL:
			if (g_application.getROM())		// Is ROM loaded?
			{
	...
	case WM_CLOSE :                     
		...
		if (NULL != GetWindow(hwndClient, GW_CHILD))	// Don't close if there is an open child
		{
			return 0;
		}
		break;
