...
WNDPROC LevelSelectDialog::ms_default_list_proc = NULL;

int LevelSelectDialog::showDialog(HINSTANCE hinst, HWND hwnd_parent)
{
    return DialogBox(hinst, MAKEINTRESOURCE(ID_DIALOG_LOAD_LEVEL), hwnd_parent, reinterpret_cast<DLGPROC>(dialogProc));
}

int LevelSelectDialog::messageCommand(HWND hwnd_dialog, UINT msg, WPARAM wparam, LPARAM lparam)
{
    if (wparam == IDOK)
    {
...
int LevelSelectDialog::messageInit(HWND hwnd_dialog, UINT msg, WPARAM wparam, LPARAM lparam)
{
    HWND hwnd_list_box = GetDlgItem(hwnd_dialog, ID_DIALOG_LOAD_LEVEL_LIST);
    ...
    Rom* pRom = g_application.getROM();
    if (!pRom)
    {
        return FALSE;
    }

    Rom::LevelNames_t& level_names = pRom->getLevelNames();
    ...
    for (Rom::LevelNames_t::iterator itr = level_names.begin(); itr != level_names.end(); ++itr)
    {
...
LRESULT CALLBACK LevelSelectDialog::dialogProc(HWND hwnd_dialog, UINT msg, WPARAM wparam, LPARAM lparam)
{
    switch(msg)
    {
    case WM_INITDIALOG:
...
LRESULT CALLBACK LevelSelectDialog::listProc(HWND hwnd_list_box, UINT msg, WPARAM wparam, LPARAM lparam)
{
    switch(msg)
    {
