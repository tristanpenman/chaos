...
public:
	...
	explicit SonicMap();
	explicit SonicMap(unsigned int layers, unsigned int width, unsigned int height);
	explicit SonicMap(unsigned int layers, unsigned int width, unsigned int height, unsigned char* data);
	...
	unsigned char setValue(unsigned int layer, unsigned int x, unsigned int y, unsigned char);    // returns old value
