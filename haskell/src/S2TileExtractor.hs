...
data Flag = Help | Level String deriving Show
...
usage argv0 = unlines [
    ...
    "Other options:",
    "  -h",
    "  -l <level>   Level to extract tiles from",
    "",
    "Example:",
    ...
    let (flags, errors) = parseOptions (tail argv) in do
        ...
        let levelIndex = getLevel flags in
            withBinaryFile (head argv) ReadMode (\handle -> do
                ...
                let patternPtrAddress = getLevelDataEntryAddress levelDataTableIndex +
                        patternPtrOffset
                let patternPtr = readPatternPtr input patternPtrAddress
                putStr $ show patternPtr
                return ())